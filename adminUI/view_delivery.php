...
if ($delivery_data != false and isset($delivery_data->{'delivery'})) {
    ...
    $delivery = false;
    echo "<p class='lead'><h2>All deliveries proccessed</h2></p>";
}


if ($delivery != false) {
    echo"<p class='lead'><h2>Deliveries not dispatched</h2></p>
    <br></br>";
    ...
    foreach ($delivery as $delivery) {
        ...
        <td style='text-align:center'><form id=searchOrder action = 'dispatch_delivery.php' method = 'post'>
            <button class='btn btn-primary btn-sm' name='invoice_id' type='submit' value= {$delivery->invoice_id}>Show More</button></td>
        </form>
        </tr>";
        }
        echo "</table>";
